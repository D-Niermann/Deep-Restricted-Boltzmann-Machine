...
def train(w,bias_v,bias_h):
	global v,h,image_vector,numpoints,_v
...
temp      = float(1) # 0.001 for heavyside function , 10 for completely random
learnrate = 1
epochs    = 1  # number of training steps“
lp        = True #if plotting while traingin is on
files_to_load=100 #files per type of class

########################################################################
########################################################################

if lp:
	fig1,ax1=plt.subplots(3,1,figsize=(5,10))
...
for i in range(epochs):
	...
	for j,image in enumerate(data):
		...
		#training
		w, bias_v, bias_h = train(w, bias_v, bias_h)

		# append variables to lists
		energy_.append(energy())
		error_.append(np.mean(image_vector-v)**2)

		if j%400==0:
			# print "Finished to ",str(round(j/len_data,2)),"%"
			if switch==0 and j/len_data>=0.05 :
				switch=1
				t2=time.clock()
				percent=float(j)/len(data)
				time_took=t2-t1
...
print "Mean Error: ",round(np.mean(error_),6)*1000, "temp: ",temp,"learnrate: ",learnrate
if lp:
	plt.close()
########################################################################################################
matrix_new=tile_raster_images(X=w, img_shape=(28, 28), tile_shape=(20, 20), tile_spacing=(1,1))
