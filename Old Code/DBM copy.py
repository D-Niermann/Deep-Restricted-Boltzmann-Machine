...
log.out("Making own train and test data")
train_data = np.zeros([1000,4])

states = np.round(rnd.random([1000]),1)

train_label=np.zeros([1000,2])
train_label[states>0.5,1] = 1
...
states = np.round(rnd.random([100]),1)

test_label=np.zeros([100,2])
test_label[states>0.5,1] = 1
test_label[states<=0.5,0] = 1
...
class DBM_class(object):
	...
	def graph_init(self,graph_mode):
		...
		self.update_l_s[-1] = self.layer[-1].assign(self.layer_samp[-1])
	...
	def fetch_hidden_save(self,N,energy_list):
		list_hidden_save = []
		for layer in range(1,self.n_layers):
			units = self.SHAPE[layer]
			a = np.zeros([N,units])
			for i in range(N):
				a[i]=self.hidden_save[i][layer-1][0]
		list_hidden_save.append(a)
	...
	def test(self,my_test_data,my_test_label,N,M,create_conf_mat, temp_start, temp_end):
		...
		temp           = temp_start
		temp_delta     = (temp_end-temp_start)/N
		...
		self.hidden_save = []
		self.energy_test = np.zeros([N,len(test_data)])
		# self.hidden_prob_save = []
		for n in range(N):
			self.layer_act[n,:] = sess.run(self.layer_activities, {self.temp_tf : temp})
			self.hidden_save.append(sess.run(self.update_l_s[1:], {self.temp_tf : temp}))
			if create_conf_mat:
				self.energy_test[n]=sess.run(self.energy)

			temp += temp_delta 
		# calc probs to smooth
		self.hidden_save.append(sess.run(self.update_l_p[1:], {self.temp_tf : temp}))
			# self.hidden_prob_save.append(sess.run(self.update_l_p[1:], {self.temp_tf : temp}))
			# sess.run(self.update_l_s[1:],{self.temp_tf : temp})
		...
		self.h1_test = self.hidden_save[-1][0]
		self.last_layer_save = self.hidden_save[-1][-1]
		...
		# 	self.class_error_test=np.mean(np.abs(self.last_layer_save-my_test_label[:,:k]))		
		...
		# 		self.conf_data = np.zeros([k,1,k]).tolist()
	...
	def gibbs_sampling(self,v_input,gibbs_steps,TEMP_START,temp_end,subspace,mode,liveplot=1):
		...
		if mode=="context":
			...
			# mod = sess.run(self.modification_tf.assign(modification))
			# log.out("mod: ",mod[0])
			...
			for step in range(gibbs_steps):
				...
				if subspace == "all":
					...
					self.class_error_gibbs_nc[step] = np.mean(np.abs(layer[-1]-input_label))
					# save layer difference to previous layers
					if step!=0:
						for i in range(self.n_layers-1):
							self.layer_diff_gibbs_nc[i,step] = np.mean(np.abs(layer[i]-layer_[i]))
				else:
					## wih context
					# save layer activites 
					self.activity_c[:,step]   = sess.run(self.layer_activities[1:], {self.temp_tf : temp})
					self.class_error_gibbs_c[step] = np.mean(np.abs(layer[-1]-input_label))
					# save layer difference to previous layers
					if step!=0:
						for i in range(self.n_layers-1):
		...
		if mode=="generate":
			...
			for step in range(gibbs_steps):
				...
				# sess.run(self.update_l_s[:-1], {self.temp_tf : temp})
				sess.run([self.update_l_s[2],self.update_l_s[1],self.update_l_s[0]], {self.temp_tf : temp})
...
N_BATCHES_PRETRAIN = 100 				# how many batches per epoch for pretraining
N_BATCHES_TRAIN    = 100 				# how many batches per epoch for complete DBM training
N_EPOCHS_PRETRAIN  = [20,20,0,0,0,0] 		# pretrain epochs for each RBM
N_EPOCHS_TRAIN     = 20  				# how often to iter through the test images
TEST_EVERY_EPOCH   = 20					# how many epochs to train before testing on the test data

### learnrates 
LEARNRATE_PRETRAIN = 0.0001				# learnrate for pretraining
LEARNRATE_START    = [	0.002,
					
						0.002]			# starting learnrates for each weight. Biases always use the [0] entry
...
digit_to_generate = 8 ## which digits to gen
...
PATHSUFFIX            = r"Sun_Jul__8_18-07-28_2018_[784, 100, 100, 10]"
...
if DO_GEN_IMAGES:
	with tf.Session() as sess:
		...
		fig_en,ax_en = plt.subplots(nn,nn,sharex="col",sharey="row")

		m=0
		
		generated_img = DBM.gibbs_sampling(label_clamp, 1000, temp*1.2 , temp, 
					mode     = "generate",
					subspace = [],
					liveplot = 0)
		...
		for i in range(nn):
			for j in range(nn):
				...
				ax_en[i,j].plot(DBM.energy_generate[::10,m])
				ax[i,j].set_xticks([])
				ax[i,j].set_yticks([])
				ax[i,j].grid(False)
				m += 1
...
if DO_CONTEXT:
	with tf.Session() as sess:
		...
		h2_no_context = DBM.gibbs_sampling(test_data[index_for_number_gibbs[:]], 100, temp , temp, 
							mode     = "context",
							subspace = "all",
							liveplot = 0)

		# # with context
		h2_context = DBM.gibbs_sampling(test_data[index_for_number_gibbs[:]], 100, temp , temp, 
							mode     = "context",
							subspace = subspace,
							liveplot = 0)
		# log.end()
		...
		for i in range(DBM.n_layers-1):
			...
			ax[0].plot(1,1,color=color,label="Layer h%i"%(i+1))
		...
		for i in range(DBM.n_layers-1):
			...
			ax[1].plot(1,1,color=color,label="Layer h%i"%(i+1))
		ax[1].plot(1,1,"k--",label="No Context")
		ax[1].plot(1,1,"k-",label="with Context")
		...
		ax[2].plot(1,1,"k--",label="No Context")
		ax[2].plot(1,1,"k-",label="with Context")
		...
		for i,digit in enumerate(subspace):
			...
			for j in range(10):
				if y_nc[j]>y_c[j]:
					ax[i].bar(j,y_nc[j],color=[0.8,0.1,0.1],label="Without Context")
					ax[i].bar(j,y_c[j],color=[0.1,0.7,0.1],label="With Context")
				else:
					ax[i].bar(j,y_c[j],color=[0.1,0.7,0.1],label="With Context")
					ax[i].bar(j,y_nc[j],color=[0.8,0.1,0.1],label="Without Context")
...
if  DO_TESTING:
	...
	# plto energy
	plt.figure()
	plt.plot(DBM.energy_test[:,::100],"k",lw=1,alpha=0.4)
	plt.plot(np.mean(DBM.energy_test,1),"r",lw=2)
	...
	for i in range(DBM.n_layers):
		...
		for direc in range(2):
			...
			ax.bar(m+versch, np.mean(np.abs(DBM.l_input[i][direc])),width=0.25,linewidth=0.2,edgecolor="k")
	...
	for i in range(13):
		# plot the input
		ax3[0][i].matshow(test_data[i:i+1].reshape(int(sqrt(DBM.SHAPE[0])),int(sqrt(DBM.SHAPE[0]))),vmin=0,vmax=1)
		ax3[0][i].set_yticks([])
		ax3[0][i].set_xticks([])
		# plot the reconstructed image		
		ax3[1][i].matshow(DBM.probs[i:i+1].reshape(int(sqrt(DBM.SHAPE[0])),int(sqrt(DBM.SHAPE[0]))),vmin=0,vmax=1)
		ax3[1][i].set_yticks([])
		ax3[1][i].set_xticks([])
		...
		for layer in range(len(DBM.SHAPE)-1):
			try:
				ax3[layer+2][i].matshow(DBM.hidden_save[-1][layer][i:i+1].reshape(int(sqrt(DBM.SHAPE[layer+1])),int(sqrt(DBM.SHAPE[layer+1]))),vmin=0,vmax=1)
	...
	for i in index_for_number_test.astype(np.int)[8][0:10]:
		...
		for layer in range(len(DBM.SHAPE)-1):
			try:
				ax3[layer+2][m].matshow(DBM.hidden_save[-1][layer][i:i+1].reshape(int(sqrt(DBM.SHAPE[layer+1])),int(sqrt(DBM.SHAPE[layer+1]))))
