...
class DBM_class(object):
	...
	def graph_init(self,graph_mode):
		...
		for i in range(len(self.layer)):
			...
			self.assign_l[i]          = self.layer[i].assign(self.sample(self.layer_ph[i]))
			...
			self.layer_diversity[i]   = tf.sqrt(tf.reduce_mean(tf.square(self.layer[i] - tf.reduce_mean(self.layer[i], axis=0))))
	...
	def test(self,my_test_data,my_test_label,N,create_conf_mat, temp_start, temp_end, using_train_data = False):
		...
		log.start("Sampling hidden %i times "%N, "temp: %f -> %f"%(np.round(temp_start,5),np.round(temp_end,5)))
	...
	def gibbs_sampling(self, v_input, gibbs_steps, TEMP_START, temp_end, droprate_start, droprate_end, subspace, mode, liveplot=1):
		...
		self.layer_gs = [None]*self.n_layers
		for l in range(len(self.layer_gs)):
			self.layer_gs[l] = np.zeros([gibbs_steps,self.batchsize,self.SHAPE[l]])
		...
		if mode=="context":
			...
			for step in range(gibbs_steps):
				...
				self.glauber_step("visible", temp, droprate, self.layer_gs, step) #sess.run(self.update_l_s[1:], {self.temp_tf : temp})
				...
				if subspace == "all":
					...
					self.class_error_gibbs_nc[step] = np.mean(np.abs(self.layer_gs[-1][step]-input_label))
					# save layer difference to previous layers
					if step != 0:
						for i in range(1,self.n_layers):
							self.layer_diff_gibbs_nc[i-1, step] = np.mean(np.abs(self.layer_gs[i][step-1] - self.layer_gs[i][step]))
				else:
					## wih context
					# save layer activites 
					self.activity_c[:,step]   = sess.run(self.layer_activities[1:], {self.temp_tf : temp})
					self.class_error_gibbs_c[step] = np.mean(np.abs(self.layer_gs[-1][step]-input_label))
					# save layer difference to previous layers
					if step!=0:
						for i in range(1,self.n_layers):
							self.layer_diff_gibbs_c[i-1, step] = np.mean(np.abs(self.layer_gs[i][step-1] - self.layer_gs[i][step]))
				...
				# self.layer_save[-1][step] = self.layer_gs[-1]
		...
		if mode=="freerunning" or mode=="generate":
			...
			return np.mean(self.layer_gs[-1][-100:,:],axis=0)
...
if DO_CONTEXT:
	with tf.Session() as sess:
		...
		for i in range(DBM.n_layers-1):
			...
			ax[0].plot(1,1,color=color,label=label_str)
...
if DO_TRAINING:
	...
	for i in range(DBM.n_layers):
		...
		plt.plot(smooth(np.array(DBM.layer_diversity_)[::2,i],10),label=label_str)
		plt.legend()
	...
	if DBM.classification:
		...
		plt.plot(x,DBM.save_dict["Class_Error_Train_Data"],"o--",label="Class Error Test\nTrain Data")
	plt.legend(loc="best")
	plt.xlabel("Update Number")
